...
# App: use shared error mapping, keep retry conflict-safe, and show failures outside the editor.
replace_once(
    "src/App.tsx",
...
retry_block = '''  const retrySaveCurrentNotes = useCallback(() => {
    clearPersistTimer();
...
# NoteEditor: use the same message mapping and clean the Phase 34 formatting artifact.
replace_once(
...
Path("src/lib/saveFailureUi.ts").write_text(
...
Path("src/lib/__tests__/saveFailureUi.test.ts").write_text(
...
Path("src/__tests__/App.global-save-failure.test.tsx").write_text(
